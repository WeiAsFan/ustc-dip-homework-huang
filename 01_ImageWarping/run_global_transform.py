...
def apply_transform(image, scale, rotation, translation_x, translation_y, flip_horizontal):
    ...
    rotation_matrix = cv2.getRotationMatrix2D(centre, rotation, scale)
    rotation_matrix[0, 2] += translation_x
    rotation_matrix[1, 2] += translation_y
    transformed_image = cv2.warpAffine(src = image, M = rotation_matrix, dsize = (height, width), borderValue = (255, 255, 255))
    if flip_horizontal:
