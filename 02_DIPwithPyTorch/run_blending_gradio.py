...
def create_mask_from_points(points, img_h, img_w):
    ...
    # 使用多边形点绘制多边形，填充内部区域
    draw.polygon([tuple(p) for p in points], outline=1, fill=255)
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    # 使用 sobel 核心来计算图像梯度，获得拉普拉斯算子效果
    sobel_kernel = torch.tensor([
        [-1, -1, -1],
        [-1, 8, -1],
        [-1, -1, -1]
    ], dtype=torch.float32).unsqueeze(0).unsqueeze(0).to(foreground_img.device)

    fg_gray = foreground_img.mean(dim=1,keepdim=True)
    bg_gray = blended_img.mean(dim=1, keepdim=True)
    # 对图像进行卷积，计算梯度
    fg_grad = F.conv2d(fg_gray, sobel_kernel, padding=1)
    bg_grad = F.conv2d(bg_gray, sobel_kernel, padding=1)

    # 计算损失，首先对前景区域和背景区域分别应用掩码
    fg_grad = fg_grad * foreground_mask  # 仅保留前景部分
    bg_grad = bg_grad * background_mask  # 仅保留背景部分

    fg_grad = fg_grad[foreground_mask.bool()]
    bg_grad = bg_grad[background_mask.bool()]
    # 拉普拉斯损失为前景和背景梯度的差异
    loss = F.mse_loss(fg_grad, bg_grad)  # 使用均方误差作为损失函数
